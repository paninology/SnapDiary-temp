...
//           let card = realm.object(ofType:T, forPrimaryKey: primaryKey) {
//            try? realm.write {
...
//        }
    ...
    func updateBook(book: Book, completion:(_ oldBook: Book)-> Void ) {
        if let realm = try? Realm(),
        let book = realm.object(ofType:Book.self, forPrimaryKey: book.objectId ) {
            try? realm.write {
                completion(book)
            }
        }
